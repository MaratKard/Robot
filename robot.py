...
def _on_order(data):
    ...
    print('OnOrder')
    orders_df = load_orders(orders_df)
    trades_df = load_trades(trades_df)
...
def load_trades(trades_df) -> pd.DataFrame:
    ...
    for curr_trade in curr_trades:
        curr_trade_row = pd.DataFrame({"Trade_ID" : curr_trade["trade_num"],
                                       ...
                                       "Trade_Type" : "Buy" if bin(curr_trade["flags"])[2:][-2] == "0" else "Sell",
